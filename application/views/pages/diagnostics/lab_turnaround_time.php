...
$(function(){
	...
	?>
		...
		<?php echo form_open("diagnostics/lab_turnaround_time/",array('role'=>'form','class'=>'form-custom')); ?>
	...
	<br />
	<?php if(isset($lab_turnaround_time) && count($lab_turnaround_time)>0){ ?>
	...
	<thead>
		...
		<th style="text-align:center">TAT Order to Update</th>
		<th style="text-align:center">TAT Update to Report</th>
		<th style="text-align:center">TAT Total</th>
	</thead>
	<tbody>
	<?php 
        $i=1;
	...
	$tests_reported=0;
	$tests_rejected=0;
	foreach($lab_turnaround_time as $turnaround){
            $tests_ordered+=$turnaround->ordered;
            $tests_completed+=$turnaround->updated;
            $tests_reported+=$turnaround->approved;
	?>
	<tr>
            <td><?php echo $i++ ;?></td>
            <td><?php echo $turnaround->test_name ;?></td>
            <td style="text-align:right"><?php echo $turnaround->ordered ;?></td>
            <td style="text-align:right"><?php echo $turnaround->updated ;?></td>
            <td style="text-align:right"><?php echo $turnaround->approved ;?></td>
            <td style="text-align:right"><?php if(!!$turnaround->updated) {$time = convertTime(round($turnaround->ordered_to_update/$turnaround->updated)); if(!!$time['hours']) echo $time['hours']."hrs "; echo $time['mins']."mins";}?></td>
            <td style="text-align:right"><?php if(!!$turnaround->approved){ $time = convertTime(round($turnaround->update_to_approve/$turnaround->approved));  if(!!$time['hours']) echo $time['hours']."hrs "; echo $time['mins']."mins";}?></td>
            <td style="text-align:right"><?php if(!!$turnaround->approved){ $time = convertTime(round($turnaround->ordered_to_approve/$turnaround->approved)); if(!!$time['hours']) echo $time['hours']."hrs "; echo $time['mins']."mins";}?></td>
	</tr>
	<?php /*
...
/*	if($this->input->post('test_master')) $test_master = $this->input->post('test_master'); else $test_master=-1;
	...
	<tfoot>
                <th></th>
		<th>Total </th>
		<th class="text-right"><?php echo number_format($tests_ordered);?></th>
		<th class="text-right" ><?php echo number_format($tests_completed);?></th>
		<th class="text-right" ><?php echo number_format($tests_reported);?></th>
		<th ></th>
		<th ></th>
		<th ></th>
	</tfoot>
