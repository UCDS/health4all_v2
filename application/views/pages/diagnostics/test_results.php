	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			...
			?>
				...
				<?php } ?>
				<?php 
					if($numeric_flag==1 || $text_range_flag==1){
				?>
				<th>Value</th>
				<th>Normal Range</th>
                <?php } ?>
					...
					<?php 
							...
							if($test['test_status'] == 2) { 
								$result.=$test['test_result']." ".$test['lab_unit']."</td>";
                                                                ...
                                                                else if($test['text_range']!=1)
                                                                    $result .="<td></td>";
							} 
							else{	
								if($text_range_flag==1)
									$result.="Test not done.</td>";
								else
									$result.="Test not done.</td><td></td>";
							}
							echo $result;
                                                }else{
                                                	if($text_range_flag==1)
					 ...
					 <?php if($text_range_flag==1)
                        				if($test['text_range']==1){ 
                        					$result ="";
							if($test['test_status'] == 2) { 
								
								if($test['range_type']==4)
										$result.="<td> ".$test['range_text']."</td>";
								else $result .="<td></td>";
							} 
							else{	
...
<?php if($assay_set==1){	
					 ...
					 <?php if($text_range_flag==1)
                        if($test->text_range==1){ 
							$result="";
							if($test->test_status == 2) {								
								if($test->test_result!=NULL && $test->range_type==4)
										$result.="<td> ".$test->range_text."</td>";
								else $result .="<td></td>";
							} 
							else{	
